...
describe('testing pbkdf2', function () {
  this.timeout(360000);
  for (const vector of pbkdf2) {
    describe(`${vector.comment} : ${JSON.stringify(vector.input)}`, function () {
      if ('error' in vector) {
        it(`should be rejected because of ${vector.error}`, async function () {
          try {
            await pbkdf2Hmac(vector.input.P, vector.input.S, vector.input.c, vector.input.dkLen, vector.input.hash);
        ...
        it(`should match ${vector.output}`, async function () {
          ...
          if (vector.input.hash === 'SHA-256') { // Let's call with the default value
            ret = await pbkdf2Hmac(vector.input.P, vector.input.S, vector.input.c, vector.input.dkLen);
          } else {
            ret = await pbkdf2Hmac(vector.input.P, vector.input.S, vector.input.c, vector.input.dkLen, vector.input.hash);
          }
